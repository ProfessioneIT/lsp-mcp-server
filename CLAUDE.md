...
- **Edits made on disk**: files are edited outside the server (by Claude Code's own tools), so `prepareFile()` calls `documentManager.syncWithDisk()` on every call for an already-open document and sends `didChange` + `didSave` when the content changed. It returns `touchedAt` when it opened or changed the document; `lsp_diagnostics` then waits up to `DIAGNOSTICS_WAIT_MS` for a fresh publish (`client.waitForDiagnostics`).
