...
- **Workspace root resolution** (`findWorkspaceRootForLanguage` in `src/utils/workspace.ts`): first the *innermost* directory matching the server's `rootPatterns` wins. Only if none match does it fall back to `LSP_WORKSPACE_ROOT`, then the *outermost* directory containing a `DEFAULT_ROOT_MARKERS` entry, then the file's own directory. So `LSP_WORKSPACE_ROOT` does not override a matching `rootPatterns` hit.
